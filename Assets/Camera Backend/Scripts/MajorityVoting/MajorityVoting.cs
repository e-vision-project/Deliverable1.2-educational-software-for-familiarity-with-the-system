...
public class MajorityVoting : AsyncBehaviour
{
    ...
    public async void PerformMajorityVoting(List<string> wordsOCR)
    {
        MajProcessEnded = false;
    ...
    private static int FindMaxVotingIndex(List<string> wordsOCR)
    {
        ...
        MasoutisManager.OCRLogs = string.Join(", ", wordsOCR.Distinct().ToList().ToArray());
        ...
        MasoutisManager.validWordsLogs = string.Join(", ", validWords.Distinct().ToArray());
     ...
     */
    public static List<string> RemoveGreekAccentSequential(List<string> words)
    {
